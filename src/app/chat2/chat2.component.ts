...
export class Chat2Component {
  ...
  constructor(private route:Router, private modalService: NgbModal, private loadingService: LoadingService, private chatService : ChatService, private http:HttpClient, private componentFactoryResolver: ComponentFactoryResolver){}
  ...
  clear_chat(){
    this.chatService.deleteChatHistory(this.userId.toString()).subscribe((
      resp: any) => {
        if (resp.status === 200) {
  ...
  askQuestion() {
    if (this.question == "") {
      ...
      this.chatService.postForDynamic(questionTemp, user_id.toString()).subscribe((
        resp: any) => {
          if (resp.status === 200) {
            if (this.dynamicBubble) {
              const factory = this.componentFactoryResolver.resolveComponentFactory(ResponseComponent);
  ...
  autoScroll() {
    ...
    if (scrollElement) {
      scrollElement.scrollTo({ top: this.scrollContainer.nativeElement.scrollHeight, behavior: 'smooth' });
    }
  }
